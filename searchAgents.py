...
def cornersHeuristic(state, problem):
    ...
    while toVisit:
        closestCornerDistance = float('inf')
        newLocation = None
        for corner in toVisit:
            x = abs(location[0] - corner[0])
            y = abs(location[1] - corner[1])
            if closestCornerDistance > (x + y):
                closestCornerDistance = x + y
                newLocation = corner
...
def foodHeuristic(state, problem):
    ...
    for food in toVisit:
        distance = mazeDistance(location, food, problem.startingGameState)
